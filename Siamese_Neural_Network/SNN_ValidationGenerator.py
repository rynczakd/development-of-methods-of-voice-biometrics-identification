"""
Data Generator for Siamese Neural Network Validation Process
"""
...
class ValidationGenerator(tf.keras.utils.Sequence):
    ...
    def __init__(self,
        ...
        print('\n\tNumber of validation datapoints: {}'.format(len(self.file_indexes)))
        print('\tRoot location: {}\n'.format(self.root))
    ...
    def __data_generation(self, speaker_labels):
        ...
        for idx, speaker_id in enumerate(speaker_labels):
            ...
            anchor_batch[idx, ] = np.load(anchor_filepath)
            positive_batch[idx, ] = np.load(positive_filepath)
